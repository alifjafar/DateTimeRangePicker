...
class DateTimeRangePickerViewModel(private val timeFormatter: TimeFormatter) {
    ...
    internal val minDate: Date by lazy {
        val currentDatePlus = DateFormat.format(
            "dd",
            DateTime.now(DateTimeZone.forTimeZone(timeZone)).plusHours(6).toDate()
        )
        val currentDate =
            DateFormat.format("dd", DateTime.now(DateTimeZone.forTimeZone(timeZone)).toDate())
        if (currentDatePlus == currentDate) {
            DateTime.now(DateTimeZone.forTimeZone(timeZone)).plusDays(1).toDate()
        } else {
            DateTime.now(DateTimeZone.forTimeZone(timeZone)).toDate()
        }
    }
